...
export class BasicVirtualizer<TOptions = Record<string, any>> {
   ...
   * 所有虚拟项数据
   */
  private geItemsWithtMemo = memoFnResult(
    (count: number, scrollMargin: number) => {
      const {
        getItemKey,
        lanes,
        paddingStart,
      ...
      const startIndex =
        this.pendingDynamicSizeIndexes.length > 0
          ? Math.min(...this.pendingDynamicSizeIndexes)
          : 0;
      ...
      for (let index = startIndex; index < count; index++) {
        ...
        const lastItem =
          lanes === 1
            ? nextItems[index - 1]
            : this.getLastItemForLane(nextItems, index);
        ...
        const size =
          dynamicSize ??
          (typeof configSize === 'function' ? configSize(index) : configSize);
  ...
  public getSize() {
    return (
      this.scrollElementRect?.[this.options.horizontal ? 'width' : 'height'] ??
      0
    );
  }
  ...
  private notifyWithMemo = memoFnResult(
    (scrolling: boolean, _startIndex?: number, _endIndex?: number) => {
